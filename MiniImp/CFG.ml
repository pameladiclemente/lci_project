...
let create_node statements edges = { id = new_node_id (); statements; edges }
...
let rec cmd_node (cmd : cmd) (exit_node : node_id) (cfg : cfg) : node_id * cfg =
  ...
  | If (cond, then_cmd, else_cmd) ->
      ...
      let else_entry, cfg_after_else =
        cmd_node else_cmd exit_node cfg_after_then
      in
      let cond_node =
        create_node [ If (cond, Skip, Skip) ] [ then_entry; else_entry ]
      in
      ( cond_node.id,
        {
          cfg_after_else with
          nodes = (cond_node.id, cond_node) :: cfg_after_else.nodes;
        } )
  ...
  | While (cond, body_cmd) ->
      ...
      (* Fictitious node for loop entry *)
      let body_entry, cfg_after_body = cmd_node body_cmd loop_node cfg in
      let cond_node =
        create_node [ While (cond, Skip) ] [ body_entry; exit_node ]
      in
      let loop_node_entry =
        { id = loop_node; statements = []; edges = [ cond_node.id ] }
      in
      ( cond_node.id,
        {
          cfg_after_body with
          nodes =
            ...
            :: (loop_node, loop_node_entry)
            :: cfg_after_body.nodes;
        } )
...
let build_cfg (program : program) : cfg =
  ...
  | Program (_, _, body_cmd) ->
      ...
      let terminal_node_obj =
        { id = terminal_id; statements = [ Skip ]; edges = [] }
      in
...
let rec vars_in_a_exp (a_exp : a_exp) : StringSet.t =
  ...
  | Add (a1, a2) | Sub (a1, a2) | Mul (a1, a2) | Div (a1, a2) | Mod (a1, a2) ->
      StringSet.union (vars_in_a_exp a1) (vars_in_a_exp a2)
...
let rec vars_in_b_exp (b_exp : b_exp) : StringSet.t =
  ...
  (* Recursive cases: combine variable sets from sub-expressions *)
  | And (b1, b2) | Or (b1, b2) ->
      StringSet.union (vars_in_b_exp b1) (vars_in_b_exp b2)
...
let defined_variables (cfg : cfg) (input : string) : bool =
      ...
      List.iter
        (fun next_id ->
          let current_predecessors =
            try Hashtbl.find predecessors next_id with Not_found -> []
          in
  ...
  let top =
    Hashtbl.fold
      (fun _ definitions computed_definitions ->
        StringSet.union computed_definitions definitions)
      block_definitions
      (StringSet.singleton input)
  in
  ...
  List.iter
    (fun (id, _) ->
      ...
      if id = cfg.entry_node then (
        ...
        Hashtbl.add dvout_table id
          (StringSet.union
             (StringSet.singleton input)
             (Hashtbl.find block_definitions id))
    ...
    List.iter
      (fun (id, _) ->
        ...
        if id <> cfg.entry_node then (
          ...
          let predecessors_ids =
            try Hashtbl.find predecessors id with Not_found -> []
          in
          let new_dvin =
  ...
  It iterates through each command, confirming that every variable in 'use_vars'
  is guaranteed to be present in 'current_defs' (which tracks assignments from 
  all valid paths flowing into this point). 
  If a required variable is missing, the process halts, errors are logged, and the 'errors' flag is raised.
          ...
          List.iter
            (fun v ->
              if not (StringSet.mem v !current_defs) then (
                ...
                in
                Printf.printf
                  "Error found in block L%d: var '%s' used in '%s' but not \
                   surely initialized\n"
                  id v cmd_str;
                errors := true))
