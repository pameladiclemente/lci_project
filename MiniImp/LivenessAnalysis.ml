...
let liveness (cfg : risc_cfg) (output : string) : (label, StringSet.t * StringSet.t) Hashtbl.t =
  ...
  while !changed do
    ...
    List.iter (fun (label, block) ->
      ...
      if not (StringSet.equal old_live_in final_new_live_in && StringSet.equal old_live_out new_live_out) 
      then (
...
let build_interference_graph (cfg : risc_cfg) (liveness_table : (label, StringSet.t * StringSet.t) Hashtbl.t) (output_var : string) 
      : interference_graph * StringSet.t =
...
let color_graph (graph : interference_graph) (registers : StringSet.t) : coloring =
  ...
  StringSet.iter (fun reg ->
    ...
    let neighbors_set = List.assoc_opt reg graph |> Option.value ~default:StringSet.empty in
    let used_colors_set =
    ...
    match List.find_opt (fun color -> 
      not (StringSet.mem color used_colors_set)) available_colors with
...
let optimize_cfg (cfg : risc_cfg) (coloring : coloring) (output_var : string) : risc_cfg =
  
  (* Find the color for a given register, defaulting to itself if not found *)
  let find_color register = List.assoc_opt register coloring |> Option.value ~default:register in
  ...
  let find_color register = List.assoc_opt register coloring_with_output |> Option.value ~default:register in
