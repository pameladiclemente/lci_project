...
type biop = AddInt | SubInt | MultInt | DivInt | ModInt | AndInt | OrInt
...
let rec bexp_in_risc (b_exp : b_exp) (register : register) : instruction list =
  ...
  | LessThan (a1, a2) ->
      let r1, r2 = (new_register (), new_register ()) in
      aexp_in_risc a1 r1 @ aexp_in_risc a2 r2
      @ [ Brop (LessThan, r1, r2, register) ]
  | LessThanEqual (a1, a2) ->
      let r1, r2 = (new_register (), new_register ()) in
      aexp_in_risc a1 r1 @ aexp_in_risc a2 r2
      @ [ Brop (LessThanEqual, r1, r2, register) ]
  | GreaterThan (a1, a2) ->
      let r1, r2 = (new_register (), new_register ()) in
      aexp_in_risc a1 r1 @ aexp_in_risc a2 r2
      @ [ Brop (GreaterThan, r1, r2, register) ]
  | GreaterThanEqual (a1, a2) ->
      let r1, r2 = (new_register (), new_register ()) in
      aexp_in_risc a1 r1 @ aexp_in_risc a2 r2
      @ [ Brop (GreaterThanEqual, r1, r2, register) ]
  | Equal (a1, a2) ->
      let r1, r2 = (new_register (), new_register ()) in
      aexp_in_risc a1 r1 @ aexp_in_risc a2 r2
      @ [ Brop (Equal, r1, r2, register) ]
...
let node_to_block (node : node) : labelled_block =
      ...
      to the appropriate successor labels *)
           | If (cond, _, _) ->
               ...
               [
                 CJump
                   ( register,
                     "L" ^ string_of_int (List.nth node.edges 0),
                     "L" ^ string_of_int (List.nth node.edges 1) );
               ]
           | While (cond, _) ->
               let register = new_register () in
               bexp_in_risc cond register
  ...
  let check_jump =
    if block_instructions <> [] && List.length node.edges = 1 then
      ...
      | _ ->
          block_instructions
          @ [ Jump ("L" ^ string_of_int (List.hd node.edges)) ]
...
let instruction_to_string = function
  ...
  | Urop (op, r1, r2) ->
      Printf.sprintf "%s %s => %s"
        (match op with Not -> "not" | Copy -> "copy")
        r1 r2
...
let load_spilled (register : register) (register_map : int StringMap.t)
    (temp_register : register) : instruction list =
...
let store_spilled (register : register) (register_map : int StringMap.t)
    (res_register : register) (destination_register : register) :
    instruction list =
  let address = StringMap.find register register_map in
  [
...
let translate_instruction (register_map : int StringMap.t)
    (kept_registers : string list) (temp_register1 : register)
    (temp_register2 : register) (instruction : instruction) : instruction list =
  (* Helper function:
  ...
  let map_register register temp_register =
    if List.mem register kept_registers then register else temp_register
  in
  ...
  | Brop (op, r1, r2, r3) ->
      ...
      let load_r1 =
        if not (List.mem r1 kept_registers) then
          load_spilled r1 register_map temp_register1
        else []
      in
      let load_r2 =
        if not (List.mem r2 kept_registers) then
          load_spilled r2 register_map temp_register2
        else []
      in
      let store_r3 =
        if not (List.mem r3 kept_registers) then
          store_spilled r3 register_map r3_reg temp_register2
        else []
      in
  ...
  | Urop (op, r1, r2) ->
      ...
      let store_r2 =
        if not (List.mem r2 kept_registers) then
          store_spilled r2 register_map r2_reg temp_register2
        else []
      in
  ...
  | Load (r_addr, r_dest) ->
      ...
      let load_addr =
        if not (List.mem r_addr kept_registers) then
          load_spilled r_addr register_map temp_register1
        else []
      in
      let store_dest =
        if not (List.mem r_dest kept_registers) then
          store_spilled r_dest register_map r_dest_reg temp_register1
        else []
      in
  ...
  | LoadI (imm, r_dest) ->
      ...
      let store_dest =
        if not (List.mem r_dest kept_registers) then
          store_spilled r_dest register_map r_dest_reg temp_register2
        else []
      in
  ...
  | Store (r_src, r_addr) ->
      ...
      let load_src =
        if not (List.mem r_src kept_registers) then
  ...
  | CJump (r, l1, l2) ->
      ...
      let load_r =
        if not (List.mem r kept_registers) then
          load_spilled r register_map temp_register1
        else []
      in
...
let translate_to_target (cfg : risc_cfg) (n : int) : risc_cfg =
  if n < 4 then
    failwith "Needed 4 registers at least (2 for Brop, 2 for spilling)";
  ...
  let update_count register =
    if register <> r_temp1 && register <> r_temp2 then
      let count =
        (Hashtbl.find_opt counting_table register |> Option.value ~default:0)
        + 1
      in
  ...
  let sort_registers =
    ...
    |> List.map fst
  in
  let sort_kept_registers =
    List.filteri (fun i _ -> i < n - 2) sort_registers
  in
  let kept_registers = r_temp1 :: r_temp2 :: sort_kept_registers in
  let spilled_registers =
    List.filter (fun r -> not (List.mem r kept_registers)) sort_registers
  in
  ...
  let memory_map =
    fst
      (List.fold_left
         (fun (acc, addr) r -> (StringMap.add r addr acc, addr + 4))
         (StringMap.empty, 1000) spilled_registers)
  in
    ...
    List.map
      (fun (label, block) ->
        let new_instrs =
          List.flatten
            (List.map
               (translate_instruction memory_map kept_registers r_temp1 r_temp2)
               block.statements)
        in
